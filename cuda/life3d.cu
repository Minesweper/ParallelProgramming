        ...
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
                for (int dz = -1; dz <= 1; dz++){
                    if (dx == 0 && dy == 0 && dz == 0)
                        continue;
                    int nx = (x + dx + N) % N, ny = (y + dy + N) % N, nz = (z + dz + N) % N;
                    alive += AT(nx, ny, nz);
                }
...
void life3d_run(int N, char *universe, int T){
    ...
    dim3 blockDim(8, 8, 8);
    dim3 gridDim((N + 7) / 8, (N + 7) / 8, (N + 7) / 8);
    ...
    for (int t = 0; t < T; t++)
    {
        ...
        // 将next复制回universe
        cudaMemcpy(d_universe, d_next, N * N * N * sizeof(char), cudaMemcpyDeviceToDevice);
    }
